...
const OrderPage: React.FC<OrderPageProps> = ({
  ...
  const groupedItems = cartItems.reduce(
    (acc: Record<string, { product: Product; quantity: number }>, item) => {
      if (acc[item._id]) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const order: any = {
      userId: sessionStorage.getItem('idUser') || null, // ou récupéré via le contexte utilisateur si tu as un système d'auth
